...
fn publish_metrics_test() {
    let counter_opts = Opts::new("xpeer_test_counter", "xpeer test counter help");
    ...
    assert_eq!("xpeer test counter help", m.get_help());
    assert_eq!("xpeer_test_counter", m.get_name());
