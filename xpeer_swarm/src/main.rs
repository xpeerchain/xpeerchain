...
use xpeer_swarm::{client, swarm::XPeerSwarm};
use std::path::Path;
...
#[structopt(
    name = "xpeer_swarm",
    author = "XPeer",
    about = "XPeer swarm to start local nodes"
)]
struct Args {
    ...
    /// Directory used by launch_swarm to output XPeerNodes' config files, logs, xpeerdb, etc,
...
fn main() {
    ...
    let swarm = XPeerSwarm::launch_swarm(
        num_nodes,
    ...
    println!("To run the XPeer CLI client in a separate process and connect to the local cluster of nodes you just spawned, use this command:");
    println!(
    ...
    if args.start_client {
        ...
        // Explicitly capture CTRL-C to drop XPeerSwarm.
