...
const LIBRA_NODE_BIN: &str = "xpeer_node";

pub struct XPeerNode {
    node: Child,
...
impl Drop for XPeerNode {
    // When the XPeerNode struct goes out of scope we need to kill the child process
    fn drop(&mut self) {
        ...
        match self.node.try_wait() {
            ...
            _ => {
                if let Err(e) = self.node.kill() {
                    panic!("XPeerNode process could not be killed: '{}'", e);
                }
            }
        }
...
#[derive(Debug)]
pub enum XPeerSwarmDir {
    Persistent(PathBuf),
    Temporary(TempDir),
}

impl AsRef<Path> for XPeerSwarmDir {
    fn as_ref(&self) -> &Path {
        match self {
            XPeerSwarmDir::Persistent(path_buf) => path_buf.as_path(),
            XPeerSwarmDir::Temporary(temp_dir) => temp_dir.path(),
        }
    }
}

/// Struct holding instances and information of XPeer Swarm
pub struct XPeerSwarm {
    // Output log, XPeerNodes' config file, xpeerdb etc, into this dir.
    pub dir: Option<XPeerSwarmDir>,
    // Maps the peer id of a node to the XPeerNode struct
    pub nodes: HashMap<String, XPeerNode>,
...
impl XPeerSwarm {
            ...
            for (node, done) in self.nodes.values_mut().zip(done.iter_mut()) {
                ...
                match node.health_check() {
                    ...
                    HealthStatus::Crashed(status) => {
                        error!(
                            "XPeer node '{}' has crashed with status '{}'. Log output: '''{}'''",
                            node.peer_id,
                            status,
    ...
    pub fn get_validator(&self, peer_id: &str) -> Option<&XPeerNode> {
    ...
    fn launch_node(
        ...
        let mut node =
            XPeerNode::launch(config, path, &logs_dir_path, disable_logging, self.tee_logs)
                .unwrap();
        for _ in 0..60 {
...
impl Drop for XPeerSwarm {
    fn drop(&mut self) {
        // If panicking, we don't want to gc the swarm directory.
        if std::thread::panicking() {
            if let Some(dir) = self.dir.take() {
                if let XPeerSwarmDir::Temporary(temp_dir) = dir {
