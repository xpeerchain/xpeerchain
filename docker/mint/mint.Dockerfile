...
RUN mkdir -p /opt/xpeer/bin /opt/xpeer/etc /xpeer/client/data/wallet/

#TODO: Remove this once wallet location is set properly
RUN mkdir -p /xpeer/client/data/wallet/

COPY --from=builder /xpeer/target/release/client /opt/xpeer/bin
COPY docker/mint/server.py /opt/xpeer/bin

# Mint proxy listening address
EXPOSE 8000

# Define TRUSTED_PEERS, MINT_KEY, AC_HOST and AC_PORT environment variables when running
CMD cd /opt/xpeer/etc && echo "$TRUSTED_PEERS" > trusted_peers.config.toml && echo "$MINT_KEY" | \
    base64 -d > mint.key && \
    cd /opt/xpeer/bin && \
