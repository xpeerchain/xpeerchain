...
//! This module provides helpers to initialize [`XPeerDB`] with fake generic state in tests.
...
pub fn db_with_mock_genesis<P: AsRef<std::path::Path>>(dir: &P) -> Result<XPeerDB> {
    let genesis_ledger_info_with_sigs = GENESIS_INFO.1.clone();
    let genesis_txn = GENESIS_INFO.2.clone();

    let db = XPeerDB::new(dir);
