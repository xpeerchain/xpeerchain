...
//! This crate provides [`XPeerDB`] which represents physical storage of the core XPeer data
//! structures.
//!
//! It relays read/write operations on the physical storage via [`schemadb`] to the underlying
//! Key-Value storage system, and implements xpeer data structures on top of it.
...
use crate::{
    errors::XPeerDbError, event_store::EventStore, ledger_store::LedgerStore, schema::*,
...
fn error_if_too_many_requested(num_requested: u64, max_allowed: u64) -> Result<()> {
    if num_requested > max_allowed {
        Err(XPeerDbError::TooManyRequested(num_requested, max_allowed).into())
    } else {
        Ok(())
    }
}
...
/// access to the core XPeer data structures.
pub struct XPeerDB {
    db: Arc<DB>,
...
impl XPeerDB {
    /// This creates an empty XPeerDB instance on disk or opens one if it already exists.
    pub fn new<P: AsRef<Path> + Clone>(db_root_path: P) -> Self {
        ...
        let path = db_root_path.as_ref().join("xpeerdb");
        let instant = Instant::now();
        let db = Arc::new(
            DB::open(path.clone(), cf_opts_map)
                .unwrap_or_else(|e| panic!("XPeerDB open failed: {:?}", e)),
        );

        info!(
            "Opened XPeerDB at {:?} in {} ms",
            path,
