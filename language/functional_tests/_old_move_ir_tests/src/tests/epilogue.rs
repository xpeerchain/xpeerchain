...
main() {
   ...
   sequence_number = XPeerAccount.sequence_number(move(sender));
...
fn charge_more_gas_on_tx_script_failure1() {
    ...
    main(initial_balance: u64, good_gas_fees: u64) {
      ...
      let sender_balance;
      sender = get_txn_sender();
      sender_balance = XPeerAccount.balance(copy(sender));
...
main(payee: address, amount: u64) {
  XPeerAccount.pay_from_sender(move(payee), move(amount));
  assert(false, 66);
  return;
}
",
...
main(recipient: address, recipient_original_balance: u64) {
   ...
   recipient_balance = XPeerAccount.balance(move(recipient));
...
main(recipient: address, original_balance: u64, gas_deposit_amount: u64) {
  ...
  sender_sequence_number = XPeerAccount.sequence_number(move(sender));
