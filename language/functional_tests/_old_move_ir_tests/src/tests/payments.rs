...
main() {
    ...
    XPeerAccount.deposit(copy(addr), move(i_created_money));
...
main() {
    ...
    sender_balance = XPeerAccount.balance(copy(addr));

    all_coins = XPeerAccount.withdraw_from_sender(move(sender_balance));

    sender_new_balance = XPeerAccount.balance(copy(addr));
    assert(move(sender_new_balance) == 0, 41);

    one_coin = XPeerAccount.withdraw_from_sender(1);
...
main() {
    ...
    sender_old_balance = XPeerAccount.balance(copy(addr));
    zero_resource = XPeerCoin.zero();
    XPeerAccount.deposit(copy(addr), move(zero_resource));

    sender_new_balance = XPeerAccount.balance(move(addr));
