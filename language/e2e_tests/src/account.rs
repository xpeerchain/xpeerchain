...
//! Test infrastructure for modeling XPeer accounts.
...
/// Details about a XPeer account.
///
/// Tests will typically create a set of `Account` instances to run transactions on. This type
/// encodes the logic to operate on and verify operations on any XPeer account.
...
impl Account {
    ...
    /// not automatically get added to the XPeer store. To add an account to the store, use
...
/// Helper methods for dealing with account resources as seen by the XPeer VM.
    ...
    pub fn read_balance(account: &Value) -> u64 {
        match account {
            Value::Struct(fields) => {
                let coin = fields
                    .get(1)
                    .expect("XPeerCoin.T must be the second field in Account");
                match &*coin.peek() {
                    ...
                    _ => panic!("account must contain XPeerCoin.T as second field"),
                }
            }
            _ => panic!("Account must be a Value::Struct"),
        }
