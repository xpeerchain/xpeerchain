...
//! The following document is a minimalist version of XPeer Wallet. Note that this Wallet does
...
//! foundation that is simple to understand and incrementally improve the XPeerWallet
...
use xpeer_crypto::hash::CryptoHash;
...
/// WalletXPeerry contains all the information needed to recreate a particular wallet
pub struct WalletXPeerry {
    mnemonic: Mnemonic,
...
impl WalletXPeerry {
    ...
    /// empty WalletXPeerry from that Mnemonic
    ...
    /// Constructor that instantiates a new WalletXPeerry from Mnemonic
    ...
    /// Function that returns the string representation of the WalletXPeerry Menmonic
    ...
    pub fn recover(input_file_path: &Path) -> Result<WalletXPeerry> {
    ...
    pub fn generate_addresses(&mut self, depth: u64) -> Result<()> {
        ...
        if current > depth {
            return Err(WalletError::XPeerWalletGeneric(
    ...
    pub fn new_address(&mut self) -> Result<(AccountAddress, ChildNumber)> {
        ...
        match self.addr_map.insert(address, child) {
            Some(_) => Err(WalletError::XPeerWalletGeneric(
    ...
    pub fn get_addresses(&self) -> Result<Vec<AccountAddress>> {
        ...
        for i in 0..self.addr_map.len() as u64 {
            match rev_map.get(&i) {
                ...
                None => {
                    return Err(WalletError::XPeerWalletGeneric(format!(
    ...
    /// Simple public function that allows to sign a XPeer RawTransaction with the PrivateKey
    ...
    pub fn sign_txn(&self, txn: RawTransaction) -> Result<SignedTransaction> {
        if let Some(child) = self.addr_map.get(&txn.sender()) {
            ...
            Err(WalletError::XPeerWalletGeneric(
...
/// WalletXPeerry naturally support TransactionSigner trait.
impl TransactionSigner for WalletXPeerry {
