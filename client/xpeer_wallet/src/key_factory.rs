...
//! XPeerWallet.
//!
//! Note that the XPeer Blockchain makes use of ed25519 Edwards Digital Signature Algorithm
//! (EdDSA) and therefore, BIP32 Public Key derivation is not available without falling back to
//! a non-deterministic Schnorr signature scheme. As XPeerWallet is meant to be a minimalist
...
use ed25519_dalek;
use xpeer_crypto::{hash::HashValue, hkdf::Hkdf};
...
impl ExtendedPrivKey {
    ...
    /// ChildNumber are not used in this iteration of XPeerWallet, but in order to
    ...
    /// Computes the sha3 hash of the PublicKey and attempts to construct a XPeer AccountAddress
    ...
    /// XPeer specific sign function that is capable of signing an arbitrary HashValue
    /// NOTE: In XPeer, we do not sign the raw bytes of a transaction, instead we sign the raw
    ...
    /// In other words: In XPeer, the message used for signature and verification is the sha3 hash
...
impl KeyFactory {
    ...
    pub fn private_child(&self, child: ChildNumber) -> Result<ExtendedPrivKey> {
        // application info in the HKDF context is defined as XPeer derived key$child_number.
        let mut le_n = [0u8; 8];
...
impl Seed {
    ...
    /// particular Mnemonic and salt. WalletXPeerry implements a fixed salt, but a user could
