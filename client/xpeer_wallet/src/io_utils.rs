...
pub fn recover<P: AsRef<Path>>(path: &P) -> Result<WalletXPeerry> {
    ...
    let mut wallet = WalletXPeerry::new_from_mnemonic(mnemonic);
...
pub fn write_recovery<P: AsRef<Path>>(wallet: &WalletXPeerry, path: &P) -> Result<()> {
