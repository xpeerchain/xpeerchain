...
use hyper;
use xpeer_wallet::{io_utils, wallet_library::WalletXPeerry};
use logger::prelude::*;
use num_traits::{
...
impl ClientProxy {
    ...
    pub fn new(
        ...
        Ok(ClientProxy {
            ...
            faucet_server,
            faucet_account,
            wallet: Self::get_xpeer_wallet(mnemonic_file)?,
    ...
    pub fn mint_coins(&mut self, space_delim_strings: &[&str], is_blocking: bool) -> Result<()> {
        ...
        let num_coins = Self::convert_to_micro_xpeers(space_delim_strings[2])?;
    ...
    pub fn transfer_coins(
        ...
        let num_coins = Self::convert_to_micro_xpeers(space_delim_strings[3])?;
    ...
    pub fn recover_wallet_accounts(
        ...
        let wallet = WalletXPeerry::recover(&Path::new(space_delim_strings[1]))?;
    ...
    fn get_xpeer_wallet(mnemonic_file: Option<String>) -> Result<WalletXPeerry> {
        ...
        let wallet = if let Ok(recovered_wallet) = io_utils::recover(&wallet_recovery_file_path) {
            ...
            let new_wallet = WalletXPeerry::new();
    ...
    fn set_wallet(&mut self, wallet: WalletXPeerry) {
        self.wallet = wallet;
    }
    ...
    fn convert_to_micro_xpeers(input: &str) -> Result<u64> {
        ensure!(!input.is_empty(), "Empty input not allowed for xpeer unit");
...
mod tests {
    ...
    #[test]
    fn test_micro_xpeer_conversion() {
        assert!(ClientProxy::convert_to_micro_xpeers("").is_err());
        assert!(ClientProxy::convert_to_micro_xpeers("-11").is_err());
        assert!(ClientProxy::convert_to_micro_xpeers("abc").is_err());
        assert!(ClientProxy::convert_to_micro_xpeers("11111112312321312321321321").is_err());
        assert!(ClientProxy::convert_to_micro_xpeers("0").is_ok());
        assert!(ClientProxy::convert_to_micro_xpeers("1").is_ok());
        assert!(ClientProxy::convert_to_micro_xpeers("0.1").is_ok());
        assert!(ClientProxy::convert_to_micro_xpeers("1.1").is_ok());
        // Max of micro xpeer is u64::MAX (18446744073709551615).
        assert!(ClientProxy::convert_to_micro_xpeers("18446744073709.551615").is_ok());
        assert!(ClientProxy::convert_to_micro_xpeers("184467440737095.51615").is_err());
        assert!(ClientProxy::convert_to_micro_xpeers("18446744073709.551616").is_err());
    }
    ...
    proptest! {
        ...
        #[test]
        fn test_micro_xpeer_conversion_random_string(req in any::<String>()) {
            let _res = ClientProxy::convert_to_micro_xpeers(&req);
        }
        #[test]
        fn test_micro_xpeer_conversion_random_f64(req in any::<f64>()) {
            let req_str = req.to_string();
            let _res = ClientProxy::convert_to_micro_xpeers(&req_str);
        }
        #[test]
        fn test_micro_xpeer_conversion_random_u64(req in any::<u64>()) {
            let req_str = req.to_string();
            let _res = ClientProxy::convert_to_micro_xpeers(&req_str);
        }
    }
