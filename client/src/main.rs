...
#[structopt(
    ...
    about = "XPeer client to connect to a specific validator"
)]
struct Args {
    ...
    /// If not passed, a new mnemonic file will be generated by xpeer_wallet in the current
    /// directory.
    ...
    /// the network. To connect to testnet, use 'xpeer/scripts/cli/trusted_peers.config.toml'.
    /// Can be generated by xpeer-config for local testing:
    /// `cargo run --bin xpeer-config`
    /// But the preferred method is to simply use xpeer-swarm to run local networks
...
fn main() -> std::io::Result<()> {
    ...
    loop {
        let readline = rl.readline("xpeer% ");
        match readline {
            Ok(line) => {
