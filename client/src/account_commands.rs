...
impl Command for AccountCommandRecoverWallet {
    ...
    fn get_description(&self) -> &'static str {
        "Recover XPeer wallet from the file path"
    }
    fn execute(&self, client: &mut ClientProxy, params: &[&str]) {
        ...
        match client.recover_wallet_accounts(&params) {
            ...
            Err(e) => report_error("Error recovering XPeer wallet", e),
        }
    }
}
...
impl Command for AccountCommandWriteRecovery {
    ...
    fn get_description(&self) -> &'static str {
        "Save XPeer wallet mnemonic recovery seed to disk"
    }
    fn execute(&self, client: &mut ClientProxy, params: &[&str]) {
        println!(">> Saving XPeer wallet mnemonic recovery seed to disk");
